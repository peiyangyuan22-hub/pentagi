...
class V11Analyzer:
    ...
    def build_strategies(self, predictions: list[Prediction]) -> StrategyOutput:
        ...
        strategy_a = self._build_a(greens)
        strategy_b = self._build_b(greens, yellows, hots)
        strategy_c = self._build_c(greens, hots)

        return StrategyOutput(a=strategy_a, b=strategy_b, c=strategy_c)

    def _build_a(self, greens: list[Prediction]) -> list:
        """方案 A: 稳健基石 — 最低赔率+最强信号 3-4 场"""
        pool = [p for p in greens if p.odd < 2.00]
        pool.sort(key=lambda p: p.odd)
        return pool[:4]

    def _build_b(self, greens: list[Prediction], yellows: list[Prediction], hots: list[Prediction]) -> list:
...
if __name__ == "__main__":
    ...
    predictions = [analyzer.analyze_match(m) for m in matches]
    ...
    for name, picks in [("A", outputs.a), ("B", outputs.b), ("C", outputs.c)]:
        ids = [p.match_id for p in picks]
        print(f"  [{name}] 场次={ids} 资金={funds[name]}")
